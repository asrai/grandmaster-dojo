...
- **REQ-834** *(Ubiquitous)*: The system shall 도전자 목록과 다음 상대를 **S7 도전자 선택 화면**이 지고, 홈(S2)에는 두지 않는다 — 도전자 정보는 그 행동의 진입 화면에 속하지 홈에 속하지 않는다. 홈은 「지금 무엇을 할 수 있나」의 요약이다. 「대련」이 별도 메뉴로 분리된 뒤로 홈 요약과 S7 은 한 탭 거리에서 같은 정보를 두 번 말한다(운영자 판단 2026-09-03).
