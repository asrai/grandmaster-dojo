...
- No build step: `index.html` is the shipped artifact — markup and styles only, with its runtime logic in the ESM modules under `src/`, entered through `<script type="module" src="src/ui/app.mjs">`. ESM is same-origin only, so it must be served over HTTP; do not open `file://`. The server is launched through `scripts/l4-serve.sh` (see the L4 bullet below), never as a bare `python3 -m http.server` — a bare one has no owner and outlives the session.
- **`src/` root is DOM-free by contract** (`balance.mjs` content tables + `balance.data.json` loader · `core.mjs` pure logic · `log.mjs` schema+buffer · `bot.mjs` bot + headless cycle driver); that is what lets the Node harness import the same modules the browser runs. Everything that touches the document lives under `src/ui/` — keep that boundary when adding modules.
- **Ten `src/ui/` modules are imported by the Node harness and therefore DOM-free too** — `sequence-input.mjs` (input engine), `match.mjs` (exchange loop), `session.mjs` (session state + log sink), `wiring.mjs` (the instrumentation hook bundles the screens and the headless cycle share), `tablet-state.mjs` (which enter/hold/exit/only state each 죽간 moves to when the candidate set changes — split out of `tablets.mjs` precisely so the transition is checkable without a document), `theme.mjs` (the display tables — palette names, 6단 자형, 화면 좌표축 — whose boot-time throws only fire if something imports them), `audio.mjs` (cue → sound-id resolution; playback is lazy so the module scope stays DOM-free and the harness can pin that no cue is silently unmapped), `frame-budget.mjs` (p95 / dropped-frame arithmetic over injected frame deltas), `stage-scale.mjs` (the stage's shrink-to-fit ratio over injected box and stage sizes, so `0 < k <= 1` is checkable without a viewport), `gesture-block.mjs` (which WebKit pinch events get blocked on an injected target, so the browser-zoom contract is checkable without a document). The authoritative list is `grep "src/ui/" tests/harness.mjs` — this sentence is a summary of it, and it went stale once already. Putting a `document` reference in any of them turns `node tests/harness.mjs` red, and `ci` with it. Anything they need from the page arrives as an injected callback (`now` · `remainingRatio` · `log` · `timer` · `instructed`); keep it that way when extending them. A screen composes its own render on top of a wiring bundle (`composeHooks`) rather than handing render callbacks to the bundle — the reverse direction would drag DOM into a module the harness imports. `src/bot.mjs` sits above the data/logic layer and below the DOM: it may import any of them, and `src/ui/screens/dispatch.mjs` imports it back for the shared disciple hand — that is the one intentional edge from `src/ui/` into `src/`.
- All tunables live in `src/balance.data.json` and reach code as the `BALANCE` object — `balance.mjs` only imports (`with { type: 'json' }`), validates, deep-freezes and re-exports it, so the 6단 판정표 + 파라미터 stay **data-driven JSON** (round-2 CTO#2) and balance tuning never touches logic.
- **Visual tokens are a separate ledger from balance tunables, and the split is by axis, not by convenience** — `src/balance.data.json` is the SoT for *game-rule* parameters (판정표 · 창 길이 · 성 계단 비용), while the palette C1~C9, type roles, 6단 판정 크기·위치, juice durations and 성 계단 규격 live in the `:root` block at the top of `index.html`'s `<style>` (REQ-810·814·817). `src/ui/theme.mjs` is the JS-side access layer over that ledger — it names tokens (`var(--c4)`) and never carries a literal colour or a tuning number. Where a rule parameter has to reach the render layer it is **injected**, never copied — `app.mjs` writes `BALANCE.buttonHitPx` into `--hit`, and `verdict-overlay.mjs` writes `BALANCE.resolveMs` into `--vd-dur` — so the value still has exactly one home. A change that would put a colour in `balance.data.json`, or copy a rule parameter into the CSS ledger instead of injecting it, is on the wrong side of the line.
- **L4 브라우저 검증은 `scripts/l4-serve.sh` 경유가 계약이다** — 서버·크롬을 즉석 명령으로 띄우면 정리를 물 자리가 없어 세션 종료 뒤 `python3 -m http.server` 와 헤드리스 크롬이 PPID=1 로 남는다(#231 에서 4일치 13개 · BGM 11겹 재생으로 발각). `run` 은 명령 하나를 감싸 `trap` 으로 걷고, `up`/`down` 은 여러 턴에 걸친 순회용이다. `kill -9`·크래시가 남긴 잔여분은 `bash scripts/l4-sweep.sh`(기본 dry-run, 종료는 `--kill`)가 사후에 걷는다 — 소유 판별은 포트가 아니라 크롬의 `--user-data-dir` 과 http.server 의 cwd 로 하며, 포트로 고르면 같은 `ps` 목록에 있는 실서비스를 죽인다. macOS 전용이다.
...
- The first tier is enforced, not just declared: `.github/workflows/ci.yml` (job `ci`) runs `node --check` over every `*.mjs`, `scripts/check-imports.mjs` (links the whole module graph without evaluating it, so a broken relative path or a missing named export fails even in the DOM-touching `src/ui/**` modules, and the `index.html` module-script `src=` entry edge is checked too), the harness, `scripts/check-font-coverage.mjs` (the shipped bundle's rendered characters vs. the `@font-face` declarations in `index.html`, so a new string that the hand-managed subset does not cover fails the PR instead of shipping as tofu), and comment-lint on every PR, and is registered as the required status check of ruleset `main protection` (id 21919517 — registration is a post-landing API step, so the evidence lives in issue #6). Renaming the job without updating that ruleset in the same session silently blocks every later PR. The remaining HTML5 workflow gaps (Chrome bot dispatch, `.gitleaks.toml`, `.editorconfig`) are in `docs/TODO-workflow-variants.md`.
- **Fonts are a subset built from the used-character set** — `assets/fonts/` holds four subset woff2 files (F1 Nanum Myeongjo 400/800 for Hangul·Latin, F2 Noto Serif KR 400/800 for Hanja) plus their OFL licenses, split inside one `'Dojo'` family by `unicode-range`. `bash scripts/subset-fonts.sh` regenerates them and is a developer-run script (`pyftsubset` is never a CI dependency — CI only reads the committed woff2). Adding text with a character the subset lacks turns the coverage gate red; that is the designed behaviour, and the fix is to re-run the script and commit its output, never to bypass the gate.
