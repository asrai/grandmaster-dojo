...
- **REQ-855** *(Event-driven)*: When 제자가 해당 수의 파해를 보유해 유도가 뜰 때, the system shall 그 죽간을 **금색 펄스**로 반짝인다 — 선택 상태의 색(청)과 같은 색을 쓰면 「유도」로 읽히지 않는다. 금은 이 프로젝트에서 달성·확정의 색이고 유도는 그 예고다. *(depends: REQ-810, REQ-811)*
...
- [ ] 케이스 8 *(REQ-855)*: 제자가 파해를 보유한 초 → 해당 죽간만 금색으로 펄스하고, 선택 상태의 청색과 구분된다
