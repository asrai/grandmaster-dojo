...
export function createVerdictOverlay() {
  ...
  function show({ mark = null, label, cls, punched = false }) {
    // 다음 예고가 열리는 순간(resolveMs 뒤)에는 이미 사라져 있어야 두 판정이 겹쳐 읽히지 않는다.
    // 히트스톱은 이 길이를 늘리지 않고 그 안에서 시작을 미룬다 — 원장의 `.verdict-pop.punched` 가 진다.
    const style = `--vd-dur:${BALANCE.resolveMs}ms`;
  ...
  return {
    ...
    showGrade(grade) {
      ...
      show({ mark: view.mark, label: gradeLabel(grade), cls: view.cls, punched: extreme });
    },
