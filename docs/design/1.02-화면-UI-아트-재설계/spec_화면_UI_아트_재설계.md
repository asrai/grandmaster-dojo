...
- **REQ-822** *(Ubiquitous)*: The system shall **상대 초식 스트립**을 **아레나 최상단 가로 스트립**에 고정한다 — 중앙은 판정 오버레이가 쓰는 자리이므로 이 스트립이 점유하지 않는다. 스트립의 내용은 **자리(창/판정)가 먼저 가르고 그 자리의 값이 뒤를 가른다** (#252): **감춤 모드의 창**은 상태 1종 「상대가 초식을 펼치려고 한다」 뿐이고 그 초가 상대 빈틈인지를 보지 않는다 — 창이 빈틈을 말하면 창 동안 상대를 감추는 계약을 화면이 스스로 어긴다. **판정 프레임**은 그 초의 상대 초식을 공개하고, 상대 초식이 없었던 초(= 그 초가 상대 빈틈이었던 초)에만 빈틈 문면을 세우되 **그 빈틈을 잡았는지로 한 번 더 가른다** (#255) — 대련은 잡은 갈래(완파) 「빈틈 — 상대의 허를 찔렀다」 · 흘린 갈래(피격) 「빈틈이었는데 아까운 기회를 놓쳤다」 2종이고, 파견은 제자가 매 초 반드시 완주해 흘린 갈래가 성립하지 않으므로 「빈틈 — 제자가 상대의 허를 찔렀다」 1종이다. 흘린 갈래에 잡은 문면을 세우면 같은 프레임의 중앙 판정이 「피격」인데 스트립은 이겼다고 말해 화면이 스스로를 부정한다. **배색은 그 갈림을 함께 따른다** (#263) — 잡은 갈래는 금색이고 흘린 갈래는 감춤과 같은 중립 먹이며 조판(가운데 정렬·굵기)은 판정 자리 그대로다. 색이 문면보다 먼저 읽히므로 문면만 가르면 같은 자기부정이 더 빠른 채널에 남는다. **예고 모드**는 초 시작에 상대 초식을 세우며, 창 빈틈 문면 2종(대련 「빈틈! — 아무 초식이나 완주하면 완파」 · 파견 「빈틈! — 제자가 연환을 잇는다」)은 **예고 모드 한정**으로 호출부가 전달한다. 자리를 비우지 않는 것이 계약이다 — 비우면 공개 순간에 조판이 튀고 상대가 아직 내지 않았다는 사실 자체가 화면에서 사라진다. *(depends: REQ-820)*
...
- **REQ-841** *(Ubiquitous)*: The system shall 구결 족자를 **기능 요소**로 쓴다 — 구절이 방향 한 개씩에 1:1 대응하고, 족자는 **3구절 창**이며 지금 칠 구절이 **둘째 줄에 고정**된 채 입력마다 목록이 한 줄 올라간다. 창 안에서는 **직전에 친 한 구절만 점등**되고 나머지는 흐려진다(첫 구절 직전과 마지막 구절 직전에는 빈 줄이 한 줄 생기고, 완주 상태에서는 마지막 구절이 첫 줄에 점등된 채 아래 두 줄이 빈다). 딜레이드 힌트(REQ-712, 수련은 0)가 이 점등으로 구현된다.
...
- **REQ-843** *(Ubiquitous)*: The system shall 수련 화면의 죽간을 **1매(수련 대상 초식)만** 표시한다 — 수련에는 후보 필터가 없고, 다른 초식을 후보처럼 늘어놓으면 「지금 무엇을 익히는가」가 흐려진다. 그 죽간의 폭은 **대련 4매 폭과 같다** — 후보 계단(REQ-824)이 없는 화면에서 최대 폭은 정보가 아니고, 남는 자리는 해설(REQ-844)이 받는다. *(depends: REQ-805)*
- **REQ-844** *(Ubiquitous)*: The system shall 비운 자리에 **초식 해설 3줄(창안 / 특성 / 파해)** 을 둔다 — 파해 1:1 대응(REQ-731)이 지금까지 비급 구결 텍스트에만 있었고(REQ-207), 수련은 그 초식만 보는 유일한 화면이라 해설의 제자리다. 「창안」 줄의 값은 § ⑨ 의 창안자 필드가 채운다. **「창안」 줄은 접히고, 「특성」·「파해」는 한 줄 + 말줄임**이다 — 창안자 이름·유파명은 길이를 통제할 수 없고, 나머지 둘은 구조가 고정이라 넘침이 「잘렸다」가 아니라 「더 있다」로 읽혀야 한다. *(depends: REQ-891)*
...
- **REQ-850** *(Ubiquitous)*: The system shall 파견 관전 화면이 **S1 아레나를 통째로 상속**하게 한다 — 레이어 스택·역광·비네트·기력 2·상대 슬롯·응수 창이 전부 같은 좌표이며, 바뀌는 것은 아레나에 서는 사람뿐이다(사부 → 제자). 상속이 곧 서사다. *(depends: REQ-821, REQ-822, REQ-823)*
...
- **REQ-860** *(Ubiquitous)*: The system shall 전수를 **사부의 시범과 제자의 따라 하기**로 연출한다 — 둘이 **서로 마주 보고**, 제자가 사부의 손짓을 보고 **거울로** 따라 한다. 좌측의 사부만 미러해 두 몸통 자산(계약상 좌향)이 그 자리에서 서로를 향한다. **승인 시안 `transmit_v2_done.png` 는 좌향 나란히이고 이 조문은 그 시안을 갈아 끼운 것이다** — 2D 옆모습 실루엣에는 시선을 그릴 수단이 없어 「등 뒤」 배치가 화면에서는 겹침으로만 읽혀 제자가 무엇을 보는지 화면이 설명하지 못했다(운영자 육안 검증 2026-09-04, #223).
- **REQ-861** *(Event-driven)*: When 전수가 실행될 때, the system shall 사부의 **왕복 손짓**(팔을 내렸다 올리는 1회)을 제자가 **0.5초 지연**으로 따라 하게 하고 **초식 수만큼 반복**한 뒤 두 팔을 대기 자세(`rotate(0)`)로 **정지**시킨다 — 「따라 한다」를 지는 것은 각도 오차가 아니라 타이밍 지연이고, 완료 신호는 나란해짐이 아니라 **정지**다(반복 구조에서 나란해짐은 매 사이클 발생해 신호가 되지 못한다). 팔을 별도 그룹으로 분리해 회전 각도 하나로 성립하므로 프레임 애니메이션(스프라이트 시트)을 필요로 하지 않는다. *(depends: REQ-860)*
...
- **REQ-865** *(State-driven)*: While 전수 화면이 열려 있는 동안, the system shall 하단 버튼 자리를 비우지 않고 **3단계**로 갈아 끼운다 — 빈 바닥은 의도와 무관하게 결손으로 읽히고, 실행 감각은 「내가 눌렀다」에서 온다(#172).

  | 단계 | 초식 목록 | 하단 버튼 | 제자 팔 |
  |---|---|---|---|
  | **전** — 아직 실행하지 않음 | 0행 | 「전수하기」(금 테두리) | 둘 다 대기 자세 |
  | **중** — 「전수하기」를 누른 뒤 연출 | 행 n 이 `0.5 + (n-1)`초에 하나씩 도착 | 「건너뛰기」(약한 테두리) | 사부의 왕복 손짓을 제자가 0.5초 지연으로 따라 함 |
  | **후** — 연출 종료·건너뛰기·전수 뒤 재진입 | 전 초식 | 「도장으로」(금 테두리) | 대기 자세로 정지 |

  「전」에서 상단 띠의 물러나기로 나가면 전수는 일어나지 않는다. 「중」은 총 길이(초식 수 × 사이클, REQ-867)에 닫히고 제자의 꼬리 0.5초만 그 뒤로 흐른다. 「건너뛰기」는 남은 행·손짓·꼬리를 즉시 전부 종료해 「후」로 보내며, 두 팔의 종점이 대기 자세라 자연 종료와 같은 자리다. 행 등장에는 짧은 불투명도 전이를 두되 `prefers-reduced-motion` 에서는 즉시다. 「후」의 재진입 갈래는 상태 규칙이지 통상 동선이 아니다 — 전수하면 도장의 「전수」가 「전수 완료」로 잠기므로 현재 UI 로는 라우터를 직접 부르는 경로에서만 도달한다. *(depends: REQ-761)*
...
- **REQ-867** *(Ubiquitous)*: The system shall 전수 연출의 총 길이를 **초식 수 × 사이클(`--tm-cycle-ms`)** 로 데이터에서 파생한다 — 초식 수는 무공 데이터가 지는 값이므로 연출 길이도 고정 상수가 아니라 거기서 나와야 한다(유운검법 4초식 = 4초). 행 n 의 도착은 제자 n번째 손짓의 시작 시점(`0.5 + (n-1)`초)이고, 조작 가능 시점(4행 확정 · 「도장으로」 · `CUE.TRANSMIT`)은 총 길이 그대로다 — 제자의 꼬리 0.5초는 여운으로만 흐른다. `prefers-reduced-motion` 과 저프레임 강등(`.flat`)에서는 손짓 반복 없이 대기 자세를 유지하고 행 등장 전이도 생략한다. *(depends: REQ-861, REQ-761)*
...
- **REQ-886** *(Ubiquitous)*: The system shall S7 의 슬롯 블록을 **표시 전용**으로 두고, 장착·해제는 **도장에서만** 일어나게 한다(REQ-736) — 장착 가능 초식이 10 종을 넘으면 「어느 칸을 비울지 → 무엇을 넣을지」의 2단 선택을 이 화면이 감당하지 못하고, 정식 경로가 이미 도장 행 버튼 하나이므로 교체구를 둘로 두지 않는다. 슬롯 표시는 대련 죽간과 같은 세로 카드(REQ-824)의 좌우 스크롤 스트립이라 칸 수가 늘어도 규격이 그대로다. 슬롯 부족은 「무엇이 없는지」가 아니라 **「없으면 무슨 일이 나는지」** 로 경고한다(주사색 → 도장에서 바꿔 돌아오면 금색 확인).
- **REQ-887** *(Ubiquitous)*: The system shall 도전자의 성을 **숫자로 표시하지 않는다**(REQ-722) — 난이도 신호는 목록 순서와 절초 유무뿐이다. 「이미 이긴 상대」는 주사색이 아니라 회색 보통 굵기로 쓴다. **파견 예고에는 목록이 없어 순서 신호가 없고**, A-1~A-3 은 이름·한자가 전부 「떠돌이 무인 流浪武人」으로 같아 띠의 이름이 상대를 식별하지 못한다 — 그 화면의 유일한 판별자는 초식 카드다 (#217). *(depends: REQ-811)*
- **REQ-888** *(Ubiquitous)*: The system shall **파견 예고 화면이 S7 브리핑 규격을 상속**하게 한다 — 목록은 두지 않고(임무는 한 번에 하나라 고를 것이 없다) 브리핑 시트가 본문을 채운다. 구성은 위에서부터 상단 띠 + 물러나기(REQ-897) · 상대 초식 카드(REQ-884 규약, 데이터는 그 차수의 임무 조합) · 절초 공개 한 줄 · **슬롯 자리에 제자 초식 스트립**(REQ-886 과 같은 세로 죽간 카드, 표시 전용, 성은 **제자 성**이며 사부 금색과 갈리게 쾌의 청 C6) · **경고 줄에 파견 잠금 사유**(REQ-836 어휘 「<초식> N성 · 권장 M성」, 열린 차수는 금색 확인) · 바닥 「파견 보내기」다. 사부가 대련 전에 보는 브리핑을 제자 파견 전에도 같은 규격으로 보게 하면 「사부 대신 제자가 대련한다」가 화면 구성으로 이어진다. **잠긴 차수도 이 화면에 들어오고**(REQ-743) 그 동안 상대 초식 카드는 그리지 않는다 — 그 몫은 CTA 바로 위의 경고 줄이 진다: 사유와 잠긴 버튼이 붙어 서야 왜 못 누르는지가 한 자리에서 읽힌다. REQ-885 는 여기서도 유지된다: 상대 초식과 제자 초식은 나란히 설 뿐 파해 대응선을 긋지 않으며 절초 하나만 예외다. **제자 초식 스트립 · 파견 잠금 경고 줄 · 「파견 보내기」는 한 덩어리로 바닥에 앵커되고, 남는 여백은 절초 공개 줄과 그 덩어리 사이에 몰린다** — S7 이 목록으로 채우는 자리를 파견은 여백으로 채우므로, 여백을 CTA 앞에 두면 두 브리핑 화면의 읽기 흐름이 갈리고 제자 블록이 엄지 도달 범위 밖으로 밀린다. 상대 초식 카드가 몇 장이든 그 앵커는 움직이지 않는다. 여백은 남는 높이일 뿐이라 내용이 늘면 0 까지 접히고, 그보다 더 늘면 시트가 통째로 본문 상자의 스크롤에 실린다 — 고를 목록이 없어 스크롤 축이 하나뿐이므로 REQ-881 의 「목록만 스크롤·CTA 고정」 2단 처분은 여기 서지 않는다. *(depends: REQ-880, REQ-881, REQ-884, REQ-886, REQ-836, REQ-856)*
...
- **REQ-894** *(Ubiquitous)*: The system shall 절초 파해 공개를 **대면 이력에 묶는다**(REQ-732 개정) — 첫 대면은 존재만 소문으로, 재대련부터 이름 + 파해 대상. **첫 대면 여부의 판별은 그 도전자의 재대련 회차가 0인지로 가른다**(REQ-734 가 이미 회차를 누적하므로 새 플래그를 두지 않는다). 개정 대상 조문은 [[spec_성_축_초식_단위_재설계]] REQ-732 이며, 「A-4 도전자 선택 화면이 뜨면 절초의 파해 대상을 공개한다」에서 대면 이력 조건이 추가된다.
...
- **REQ-911** *(Ubiquitous)*: The system shall 죽간과 도전자 행을 `<button>` 으로 렌더하고, 잠긴 버튼에 `disabled` 를 준다 — 현행 `pad.mjs` 가 이미 `<button>` 이므로 `<div>` 렌더는 조작 후퇴다. *(depends: REQ-819)*
...
| S4 | 파견 예고(S7 브리핑 파생) | REQ-888 · REQ-884 · REQ-886 · REQ-836 · REQ-885 | — |
...
| S5 | 실루엣 2인·동작(E3·E4·E11) | REQ-860 · REQ-861 · REQ-867 · REQ-932 | — |
...
- 접근성 — 히트 44px · 시맨틱 요소 · 대비 상향
- 에셋 이관 — 폰트 4파일 + 아이콘 1파일 + 실루엣 12종 PNG 교체
- 검증 스파이크 1건 — 저사양 실기 판정 프레임 예산 측정

### Phase 2 이관 (범위 밖)

- **REQ-806 · REQ-807 폐기** — 2026-09-03 운영자 판정으로 폐기(낭독기 미지원). 코드의 해당 배선과 REQ 앵커는 존치한다.
...
- [ ] 케이스 13 *(REQ-886)*: A-4 브리핑의 슬롯 칸은 탭해도 바뀌지 않고, 도장에서 바꾼 뒤 돌아오면 경고가 주사색에서 금색 확인으로 바뀐다
...
> **케이스 19·20 삭제 근거**: 2026-09-03 운영자 판정으로 「일대종사」는 낭독기(스크린 리더)를 지원하지 않는다. 두 케이스는 낭독을 귀로 듣는 검증이라 검증 대상 자체가 사라졌고, 그 근거였던 판정 낭독 조문도 § Phase 2 이관에서 폐기됐다. REQ-911 은 낭독 전용 어구만 걷고 `<button>`/`disabled` 조작 계약으로 남는다 — 키보드 조작과 클릭 차단은 낭독기와 무관한 조작 계약이라 존치한다. REQ-910(44px 히트 영역)은 접근성이 아니라 터치 usability 이므로 #125 § ③ 에 살아 있다.
