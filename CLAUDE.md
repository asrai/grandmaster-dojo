...
- **Three `src/ui/` modules are imported by the Node harness and therefore DOM-free too** — `sequence-input.mjs` (input engine), `match.mjs` (exchange loop), `session.mjs` (session state + log sink). Putting a `document` reference in any of them turns `node tests/harness.mjs` red, and `ci` with it. Anything they need from the page arrives as an injected callback (`now` · `remainingRatio` · `log` · `timer`); keep it that way when extending them. `src/bot.mjs` sits above the data/logic layer and below the DOM: it may import those three, and `src/ui/screens/dispatch.mjs` imports it back for the shared disciple hand — that is the one intentional edge from `src/ui/` into `src/`.
