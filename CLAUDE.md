...
- **The Node harness imports a set of `src/ui/` modules, and the authoritative list is `grep "src/ui/" tests/harness.mjs`** — a hand-maintained enumeration used to stand here and kept going stale, so the grep is now the only list. **Being on that list does not by itself make a module DOM-free**: `src/ui/arena.mjs` is not (`foeView` builds nodes through `el()` from `./dom.mjs`), and the harness imports it anyway because no document access runs at module scope — so the Node load is safe — and it calls only that module's pure functions (`foeStripState` · `FOE_STRIP`). The contract is therefore about reachability, not membership: a document reference that runs when a harness-imported module is loaded, or inside a function the harness calls, turns `node tests/harness.mjs` red, and `ci` with it. Anything the harness-facing logic needs from the page arrives as an injected callback (`now` · `remainingRatio` · `log` · `timer` · `instructed`); keep it that way when extending them. A screen composes its own render on top of a wiring bundle (`composeHooks`) rather than handing render callbacks to the bundle — the reverse direction would drag DOM into a module the harness imports. `src/bot.mjs` sits above the data/logic layer and below the DOM: it may import any of them, and `src/ui/screens/dispatch.mjs` imports it back for the shared disciple hand — that is the one intentional edge from `src/ui/` into `src/`.
