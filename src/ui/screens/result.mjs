...
function duelLines(session, params) {
  ...
  // 재대련 무보상은 규칙이라 이유를 함께 말한다 — 「+0 냥」만 뜨면 결함으로 읽힌다 (REQ-734).
  const lines = [rematch ? '재대련 승리 — 재화는 없다 (성과 계단만 남는다)' : `재화 +${reward} 냥`];
...
function dispatchLines(session, params) {
  ...
  const lines = [params.win ? `재화 +${reward} 냥` : '도전자는 도주했다 — 잃은 것은 없다'];
  return lines;
}
  ...
  composeScreen(ctx, {
    ...
    body: [
      el('div', { class: 'result-stage' }, [
        ...
        el('p', { class: 'dim', text: `${view.exchange}초를 겨뤘다 · 남은 HP ${view.selfHp} 대 ${view.foeHp}`
          + `${view.outcome.by === 'exchanges' ? ' (초 상한 · 잔여 HP 비교)' : ''}` }),
      ]),
