...
| S7 | 도전자 선택 | A-1~A-4 + 재대련 목록(회차·강화) + 절초 파해 공개 + 슬롯 표시(교체는 § 추가 결정으로 철회). **S2 v2 에서 신설** — 홈이 목록을 감당하지 못해 분리됐다 | 3 | (현행 빌드 없음 — 신규 설계) |
| S4-1 | 파견 예고 | 제자를 내보내기 전의 브리핑. **S7 브리핑 규격 파생** — 목록 없음, 슬롯 자리에 제자 초식, 경고 줄에 파견 잠금 사유, 바닥에 「파견 보내기」. 재설계 7화면에 들지 않아 v0 카드 크롬이 남아 있던 자리다(REQ-888) | 4 | `12` |
...
| | **정경의 제자는 사부 오른쪽, 「제자」 표찰은 우측 하단** — 제자 실루엣만 화면에서 좌우 반전(CSS `scaleX(-1)`)해 사부를 보게 한다 | 한때 「제자를 사부 왼쪽으로, 표찰을 그 발치로」(#175)를 채택했으나 실기에서 표찰이 제자 그림에 겹치고 정보가 좌측에 쏠려 우측이 비었다 — 운영자 판정으로 배치를 되돌리고(#213), 우향 자산이 사부를 보는 문제는 화면 반전으로 푼다. 자산 파일은 계약(우향)을 지키고 있으므로 건드리지 않는다 |
...
| 축 | 무엇을 | 왜 |
|---|---|---|
| **정서** | **사부가 초식을 시범하고 제자가 그것을 따라 한다**(E3·E4·E11) — 둘 다 같은 방향을 보고, 제자는 사부의 등 뒤에서 같은 자세를 잡는다. `follow` 상태에서 제자의 팔은 아직 34° 어긋나 있고, `done` 에서 사부와 나란해진다 | **v1 의 전수 광선을 기각한 자리다.** 광선은 「데이터가 전송된다」는 은유라 이 게임이 파는 사제 관계와 어긋난다. 무공이 넘어가는 실제 그림은 **보고 따라 하는 것**이고, 그것이 이 게임의 코어(손으로 익힌다)와도 같은 동작이다. 자세의 어긋남 → 일치가 전수의 전후를 말한다. 운영자 판단 2026-09-02 — **구도와 완료 신호는 § 추가 결정(2026-09-04, #223)이 개정** |
...
| **자산** | 신규 아트 0 — 정경 SVG·실루엣 2인 전부 코드 렌더. 팔을 별도 `<g class="arm">` 로 분리해 **회전 각도 하나로 시범/따라 하기를 전환**한다 | 구현 시 프레임 애니메이션 없이 CSS `transform: rotate()` 트랜지션만으로 이 연출이 성립한다는 뜻이다. 스프라이트 시트는 § 잔여 논의로 남는다 — **1회 전이가 유한 반복 keyframe 으로 바뀐 것은 § 추가 결정(2026-09-04, #223)** |

### 추가 결정

- **전수 화면을 3단계로 나눈다 — 진입이 아니라 화면 안의 「전수하기」 버튼이 전수를 실행한다.** 「전」은 목록 0행 + 「전수하기」(금 테두리)이고, 그 버튼을 눌러야 팔 전이가 시작되며 초식 행이 하나씩 도착한다(「중」, 바닥은 「건너뛰기」). 「후」는 종전과 같다. 종전에는 화면 진입 자체가 전수 실행이라 2초 남짓의 연출이 끝나면 들어오자마자 「도장으로」가 서 있는 것처럼 보였고, **유저는 자기가 전수했다는 감각을 받지 못했다** — 실행 감각은 연출 시간이 아니라 자기 손으로 눌렀는가에서 온다(연출 시간만 늘리는 안을 그래서 기각했다). 도장의 「전수」 버튼에 확인 다이얼로그를 붙이는 안은 무협 톤에 시스템 다이얼로그가 없어(D-1) 기각. 부수 효과로 「전」의 물러나기(E1-1)가 처음으로 뜻을 갖는다 — 전수하지 않고 나간다. 운영자 판단 2026-09-03.
- **연출을 사부-제자 왕복 손짓의 반복으로 바꾸고, 두 사람을 마주 보게 한다.** 1회성 전이는 화면에서 「손짓」으로 읽히지 않았고 사부는 아예 움직이지 않았다(운영자 육안 검증 2026-09-04). 사부가 1초 주기로 팔을 내렸다 올리고 제자가 **0.5초 지연**으로 같은 동작을 따라 하며, 이를 **초식 수만큼** 반복한 뒤 두 팔이 대기 자세로 **정지**한다 — 「따라 한다」를 지는 것은 각도 오차가 아니라 타이밍이고, 반복 구조에서는 나란해짐이 매 사이클 발생해 신호가 되지 못하므로 완료 신호를 **정지**가 진다. 연출 길이는 고정값이 아니라 **초식 수 × 사이클**로 데이터에서 파생한다. 구도는 **서로 마주 보기**로 바꿨다 — 승인 시안 `transmit_v2_done.png` 의 좌향 나란히를 갈아 끼운 것이며, 2D 옆모습 실루엣에는 시선을 그릴 수단이 없어 「등 뒤」 배치가 겹침으로만 읽혀 제자가 무엇을 보는지 화면이 설명하지 못했기 때문이다. 마주 보고 **거울로** 따라 하는 것은 초심자 교습의 표준이라 무협 톤과도 충돌하지 않는다. 기각한 대안 — 「제자의 각도 오차가 회차마다 수렴」: 회차당 각도 차이가 화면상 약 4px 이라 실루엣에서 읽히지 않아 복잡도만 는다. 운영자 판단 2026-09-04 (#223, #205 흡수).
...
> **현행 빌드에 대응 화면이 없다 — 진단 없이 신규 설계다.** 이 화면은 S2 도장 v2 가 「목록은 그 행동의 진입 화면에 속한다」로 재대련 목록을 홈에서 분리하면서 신설됐고, 1.03 이 요구하는 절초 파해 공개(REQ-732)의 착지점이기도 하다 — 함께 왔던 슬롯 in-place 교체는 § 추가 결정이 철회했고 REQ-736 도 그에 맞춰 개정됐다.
...
| **정보 공개** | **겪어본 만큼만 보여준다** — 첫 대면에는 E3 상대 초식이 붙지 않고 **E7 첫 대면 안내**가 대신 선다(「겪어본 적 없는 상대다 / 답은 다음 대면에, 슬롯은 도장에서 바꾼다」 — 문면은 § 추가 결정이 개정). 목록 행의 속성 칩도 「미상」으로 접힌다 | 싸워본 적 없는 상대의 초식을 이미 알고 있는 것은 **논리적으로 성립하지 않는다.** 운영자 판단 2026-09-02 |
...
- **슬롯 in-place 교체(E5)를 철회한다 — S7 의 슬롯은 표시 전용이다.** 위 「슬롯 교체를 이 화면 안에서 끝낸다」를 대체한다. 「판단의 순간과 조작의 장소를 붙인다」는 슬롯 3 · 초식 4 의 프로토타입에서 성립한 근거였고, 사부 레벨에 따라 장착 가능 초식이 10 종을 넘으면 「어느 칸을 비울지 → 무엇을 넣을지」의 2단 선택이 한 화면에 얹히지 않는다. 장착·해제의 정식 경로는 도장 행 버튼(REQ-736)이라 교체를 빼도 기능 손실이 없고, 물러나기(E1-1)가 이미 도장으로 간다. 아울러 슬롯 표시를 가로 3등분 그리드에서 **대련 죽간과 같은 세로 카드(84×146)의 좌우 스크롤 스트립**으로 바꾼다 — 칸 수가 늘어도 폭이 아니라 스크롤이 받으므로 393px 에서 이름이 깨지지 않는다. 운영자 판단 2026-09-03.
