...
- **REQ-865** *(State-driven)*: While 전수 화면이 열려 있는 동안, the system shall 하단 버튼 자리를 비우지 않고 **3단계**로 갈아 끼운다 — 빈 바닥은 의도와 무관하게 결손으로 읽히고, 실행 감각은 「내가 눌렀다」에서 온다(#172).
  ...
  「전」에서 상단 띠의 물러나기로 나가면 전수는 일어나지 않는다. 「건너뛰기」는 남은 행을 즉시 전부 띄운다 — 팔은 「전수하기」를 누른 시점부터 이미 종점을 향하고 있으므로 건너뛰기가 앞당기지 않으며, 그래서 종점이 자연 종료와 같다. 행 등장에는 짧은 불투명도 전이를 두되 `prefers-reduced-motion` 에서는 즉시다. *(depends: REQ-761)*
...
- [ ] 케이스 9 *(REQ-861, REQ-865)*: 전수 화면 진입 → 목록 0행 + 「전수하기」. 그 버튼을 누름 → 제자의 팔이 사부와 나란해지는 전이가 일어나고 행이 하나씩 도착하며 바닥은 「건너뛰기」, 완료 후 「도장으로」. 연출 중 「건너뛰기」를 누르면 즉시 전 행 + 「도장으로」. 전수 뒤 재진입은 「후」 상태로 바로 열린다. `prefers-reduced-motion` · `.flat` 에서는 행 등장 전이가 없다
