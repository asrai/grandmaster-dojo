...
- No build step: `index.html` is the shipped artifact and draws its runtime logic from the ESM modules in `src/` (`<script type="module">`). ESM is same-origin only, so always serve with `python3 -m http.server 8000`; do not open `file://`.
