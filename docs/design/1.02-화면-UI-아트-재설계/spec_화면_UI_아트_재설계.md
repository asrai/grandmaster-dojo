...
- **REQ-865** *(State-driven)*: While 전수 화면이 열려 있는 동안, the system shall 하단 버튼 자리를 비우지 않고 **3단계**로 갈아 끼운다 — 빈 바닥은 의도와 무관하게 결손으로 읽히고, 실행 감각은 「내가 눌렀다」에서 온다(#172).
  ...
  | **중** — 「전수하기」를 누른 뒤 연출 | 초식 수로 균등 분배해 하나씩 등장 | 「건너뛰기」(약한 테두리) | 나란해지는 전이 중 |
  | **후** — 연출 종료·건너뛰기·전수 뒤 재진입 | 전 초식 | 「도장으로」(금 테두리) | 나란함 |

  「전」에서 상단 띠의 물러나기로 나가면 전수는 일어나지 않는다. 「건너뛰기」는 남은 행을 즉시 전부 띄우고 팔을 종점으로 보내며, 그 종점은 자연 종료와 같다. 행 등장에는 짧은 불투명도 전이를 두되 `prefers-reduced-motion` 에서는 즉시다. *(depends: REQ-761)*
...
- [ ] 케이스 9 *(REQ-861, REQ-865)*: 전수 화면 진입 → 목록 0행 + 「전수하기」. 그 버튼을 누름 → 제자의 팔이 사부와 나란해지는 전이가 일어나고 행이 하나씩 도착하며 바닥은 「건너뛰기」, 완료 후 「도장으로」. 전수 뒤 재진입은 「후」 상태로 바로 열린다
