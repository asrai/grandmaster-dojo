...
- **REQ-834** *(Ubiquitous)*: The system shall 도전자 목록도 **다음 상대 요약 블록**도 홈(S2)에 두지 않고 **S7 도전자 선택 화면**이 지게 한다 — 도전자 정보는 그 행동의 진입 화면에 속하지 홈에 속하지 않는다. 홈은 「지금 무엇을 할 수 있나」의 요약이다. 「대련」이 별도 메뉴로 분리된 뒤로 홈 요약과 S7 은 한 탭 거리에서 같은 정보를 두 번 말한다(운영자 판단 2026-09-03). **예외는 하단 액션 바의 대상 표기 하나**다 — 「대련」 버튼 부제가 대는 상대 이름·차수는 그 버튼이 어디로 가는지의 안내이고 REQ-895 의 적용 대상이다. 그 부제는 버튼이 잠기면 잠금 사유로 바뀌므로(REQ-836), 홈이 도전자를 한 자도 대지 않는 국면이 있다 — 그 국면의 다음 행동은 슬롯 안내문이 진다.
- **REQ-835** *(Ubiquitous)*: The system shall S7 의 **도전자 목록 행과 브리핑이 같은 공개 층**을 읽게 한다 — 한 화면 안에서 목록이 「미상」이라 접은 상대를 브리핑이 이름으로 말하면 예고가 함정이 된다. 공개 층의 파생도 그 표시 조각도 한 벌만 둔다. *(depends: REQ-834, REQ-894)*
- **REQ-836** *(Unwanted)*: If 파견이 성 미달로 잠겨 있으면, then the system shall 버튼에서 **잠긴 이유까지 읽히게** 한다 — 「행운유수 3성 · 권장 5성」. 하드 잠금(REQ-743)의 이유가 같은 자리에서 끝나야 한다. 잠김 **상태** 자체는 자물쇠 아이콘(C5)으로도 표시한다 — 흐림 하나는 「아직 안 눌렀다」로도 읽혀 이유 문면에 닿기 전에 상태를 놓친다. 비활성이어도 더 열 것이 없는 **완료**에는 자물쇠를 두지 않는다.
...
- **REQ-931** *(Ubiquitous)*: The system shall 목업의 되돌리기 아이콘 실파일(`mockups/assets/reset.svg`)을 **초기 에셋으로 그대로 채용**한다 — S1 입력 되돌리기 · S3 입력 되돌리기. 코드는 파일 경로가 아니라 아이콘 id 를 참조해, 후속 아트 개선이 파일 스왑으로 닫히게 한다. 같은 관례로 자물쇠 아이콘(`assets/icons/lock.svg`, id `lock`)이 S2 하단 밴드의 잠김 표시를 진다(REQ-836). *(depends: REQ-829)*
...
| S2 | 유도 툴팁(E6) | REQ-912 | — |
| S2 | 하단 액션 바(E7~E7-2) | REQ-836 · REQ-895 · REQ-834 · REQ-931 | — |
| S2 | 제자 블록(E9~E9-4) | REQ-833 | — |
...
| S7 | 도전자 목록(E2~E2-5) | REQ-880 · REQ-881 · REQ-887 · REQ-834 · REQ-835 | — |
| S7 | 상대 초식 브리핑(E3~E3-2) · 절초 공개/소문(E4) | REQ-882~885 | — |
| S7 | 내 슬롯(E5~E5-3) · 대련 시작(E6) · 첫 대면 안내(E7) | REQ-886 · REQ-882 | — |

**은퇴 요소는 이관 대상이 아니다** — S2 E3-2/E3-3/E3-4/E4-5/E4-6/E5/E8/E10 · S3 E1-3 · S5 E5/E8/E9 · S7 E2-2. 각각 성 축 소멸(1.03)·중복·D-8·명시 삭제 결정으로 은퇴했다.
...
- [ ] ~~케이스 19 *(REQ-910, REQ-911)*: 키보드만으로 죽간·도전자 행 조작 → 포커스가 도달하고 낭독되며, 잠긴 버튼은 포커스를 받지 않는다~~ — **스코프 밖** (2026-09-03 운영자 판단, #125)
- [ ] ~~케이스 20 *(REQ-806, REQ-807)*: 화면을 여러 번 전환한 뒤 판정 발생 → 판정이 스크린 리더로 낭독된다~~ — **스코프 밖** (2026-09-03 운영자 판단, #125)
...
> **스코프 밖 처리 근거 (케이스 19·20)**: 「시각장애 플레이 지원」은 본 spec 의 목표로 선언된 적이 없고 계획에도 없다. 낭독기(스크린 리더)를 켜고 듣는 검증은 #125 에서 전량 삭제됐고, 두 케이스는 그 검증 없이는 닫히지 않으므로 사람 검증 대상에서 뺀다. **REQ-806·REQ-807·REQ-911 의 구현은 그대로 남는다** — 라이브 리전 분리와 `<button>`/`role="radio"`/`disabled` 시맨틱은 이미 서 있고 자동 검사가 지키며, 되돌리면 DOM 시맨틱 후퇴다. 빠진 것은 *사람이 귀로 듣는 확인 절차*뿐이다. REQ-910(44px 히트 영역)은 접근성이 아니라 터치 usability 이므로 #125 § ③ 에 살아 있다.
