...
export function createVerdictOverlay() {
  ...
  function show({ mark, label, color }) {
    ...
    const parts = mark
      ? [el('b', { class: 'verdict-mark', text: mark }), el('span', { class: 'verdict-label', text: label })]
      : [el('span', { class: 'verdict-label lead', text: label })];
    clear(node).appendChild(el('div', { class: 'verdict-pop', style: css }, parts));
    announce(label);
  }
  ...
  return {
    ...
    showGrade(grade) {
      ...
      show({ ...view, label: gradeLabel(grade) });
    },
